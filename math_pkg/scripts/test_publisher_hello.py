...
def talker():
    ...
    agmat = scipy.io.loadmat("ag1hello.mat")
    wgmat = scipy.io.loadmat("wg1hello.mat")
    quamat = scipy.io.loadmat("quatg1hello.mat")
    ag = agmat["ag1"]
    wg = wgmat["wg1"]
    qua = quamat["quatg1"]
    ...
    while True:
        ...
        imu.angular_velocity.x = wg[0,K]*0
        imu.angular_velocity.y = wg[1,K]*0
        imu.angular_velocity.z = wg[2,K]*0
        if K < 350:
            K = K + 1
        pub.publish(imu)
        rate.sleep()
